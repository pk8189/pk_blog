...
Data filtration often includes pulling data from a third-party resource to perform analysis, but
third-party APIs don't always allow for a query specific enough to fit the engineer's needs.
If this is the case, filtering the return value is necessary before moving the data any further.
...
It's important for the engineer to understand the pros 
and cons of each filtration method. It's also important
to note that the output type of the result `type(filtered_data)` of Filtration methods 1-3
is `<class 'list'>`, but type 4 is `<class 'generator'>`.
